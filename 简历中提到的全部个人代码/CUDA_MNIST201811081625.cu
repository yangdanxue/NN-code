...
当前版本进度： 【CNN-784-784-196-10@1】计算CNN多核时【CUDA_MNIST201811102304（202012032257）.cu】，发现了问题，修改【pPD_NetOutputToSigmoidSubsampling2[i*196 + (Y*14+X)]】后，正确率稳定增长，且能突破90%！！！之前误写成【pPD_NetOutputToSigmoidSubsampling2[(Y*14+X) * M + i]】。。。
*****************************************************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
//	    	pSigmoidSubsampling2[Y*14+X] = pNetSubsampling2[Y*14+X].j;		//验证i、j值都正确啦（与【pNetSubsampling2[Y*14+X].i = i】与【pNetSubsampling2[Y*14+X].j = j】两行代码匹配）。
	...
	if (X < M && Y == 0) 
	{
		for (int i = 0; i < 196; i++)
		{
	    	...
	    	///////////////////////////	【反向传播】池化2层---->输出层的权值更新（链式法则）（开始）	///////////////////////////
	   	pPD_ErrorToSigmoidOutput[X] = -(pTarget[X] - pSigmoidOutput[X]);    	

	    	///////////////////////////【反向传播】池化2层---->输出层的权值更新——SigmoidOutput-->NetOutput///////////////////////////
	    	pPD_SigmoidOutputToNetOutput[X] = pSigmoidOutput[X] * (1 - pSigmoidOutput[X]);
	    	
	    	///////////////////////////【反向传播】池化2层---->输出层的权值更新——Error-->NetOutput(输出层的误差值＝总误差对输出层神经元值求偏导)///////////////////////////
		pDelta_NetOutput[X] = pPD_ErrorToSigmoidOutput[X] * pPD_SigmoidOutputToNetOutput[X];
    	}
	__syncthreads();


	///////////////////////////【反向传播】池化2层---->输出层的权值更新——NetOutput-->WeightSubsampling2ToOutput///////////////////////////	
	int index = Y*14+X;
   	while ((index < (M*S)) && X<14 && Y<14)  
	{
		pPD_NetOutputToWeightSubsampling2ToOutput[index] = pSigmoidSubsampling2[Y*14+X];
		index += (blockDim.y * gridDim.y)/2*(blockDim.x * gridDim.x)/2;
	}
	__syncthreads();


	///////////////////////////【反向传播】池化2层---->输出层的权值更新——Error-->WeightSubsampling2ToOutput///////////////////////////	
	if (X<14 && Y<14) 
	{
 	     	for (int i = 0; i < M; i++) 
		{
			pPD_WeightSubsampling2ToOutput[i * S + Y*14+X] = pDelta_NetOutput[i] * pPD_NetOutputToWeightSubsampling2ToOutput[i * S + Y*14+X];
        	}
        }
	__syncthreads();


	///////////////////////////【反向传播】池化2层---->输出层的权值更新——更新WeightSubsampling2ToOutput的值（新权重占10%，旧权重占90%）///////////////////////////
	index = Y*14+X;
	while ((index < (M*S)) && X<14 && Y<14)
	{
		pWeightSubsampling2ToOutput[index] = (pWeightSubsampling2ToOutput[index])*OldWeightPercentage + 
												(pWeightSubsampling2ToOutput[index] - LearningRate * pPD_WeightSubsampling2ToOutput[index])*NewWeightPercentage;
		index += (blockDim.y * gridDim.y)/2*(blockDim.x * gridDim.x)/2;
	}
	__syncthreads();


	///////////////////////////【反向传播】池化2层---->卷积1层的权值更新——NetOutput-->SigmoidSubsampling2///////////////////////////	
	index = Y*14+X;
	while ((index < (M*S)) && X<14 && Y<14)
	{
		pPD_NetOutputToSigmoidSubsampling2[index] = pWeightSubsampling2ToOutput[index];
		index += (blockDim.y * gridDim.y)/2*(blockDim.x * gridDim.x)/2;
	}
	__syncthreads();


	///////////////////////////【反向传播】池化2层---->卷积1层的权值更新——SigmoidSubsampling2-->NetSubsampling2///////////////////////////
	if (X<14 && Y<14) 
	{
		pPD_SigmoidSubsampling2ToNetSubsampling2[Y*14+X] = 1;
	}
	__syncthreads();
    	...
    	if (X<14 && Y<14) 
	{
	     	for (int i = 0; i < M; i++) 
		{
           		tmpSum += pPD_NetOutputToSigmoidSubsampling2[i*196 + (Y*14+X)] * pDelta_NetOutput[i];//计算CNN多核时【CUDA_MNIST201811102304（202012032257）.cu】，发现了问题，之前误写成【pPD_NetOutputToSigmoidSubsampling2[(Y*14+X) * M + i]】
        	}
    	    	pDelta_Subsampling2[Y*14+X] = tmpSum;
	}
	__syncthreads();
	...
	if (X < 14 && Y < 14) 
	{
		for(int j = 0; j < 2; j++)
		{
			for (int i = 0; i < 2; i++)
			{
				pUpsample_Delta_Subsampling2[(Y*2+j)*28+(X*2+i)] = 0;
			}
		}
		pUpsample_Delta_Subsampling2[((Y*2+(int)pNetSubsampling2[Y*14+X].j)*28+(X*2+(int)pNetSubsampling2[Y*14+X].i))] = pDelta_Subsampling2[Y*14+X];	//值正确，位置正确，全部正确！！！
	}
	__syncthreads();


    	///////////////////////////【反向传播】池化2层---->卷积1层的权值更新——SigmoidConvolution1-->NetConvolution1///////////////////////////	
	if (X < 28 && Y < 28) 
	{
		pPD_SigmoidConvolution1ToNetConvolution1[Y*28+X] = pSigmoidConvolution1[Y*28+X] * (1 - pSigmoidConvolution1[Y*28+X]);
     	}
   	__syncthreads();
    	...
    	if (X < 14 && Y < 14) 
	{
		for(int j = 0; j < 2; j++)
		{
			for (int i = 0; i < 2; i++)
			{
				pDelta_Convolution1[(Y*2+j)*28+(X*2+i)] = pUpsample_Delta_Subsampling2[(Y*2+j)*28+(X*2+i)] * pPD_SigmoidConvolution1ToNetConvolution1[(Y*2+j)*28+(X*2+i)];
			}
		}
	}
   	__syncthreads();
	...
	if (X < 5 && Y < 5) 
	{
	 	for (int j = 0; j < 28; j++) 
		{
			for (int i = 0; i < 28; i++) 
			{
				...
				tmpSum += pPadding_NetInput[(Y+j)*32+(X+i)] * pDelta_Convolution1[j*28+i];
			}
		}
		pPD_WeightInputToConvolution1[Y*5+X] = tmpSum;
	}
   	__syncthreads();
   	

	///////////////////////////【反向传播】卷积1层---->输入层（卷积核）的权值更新——更新WeightInputToConvolution1的值（新权重占10%，旧权重占90%）///////////////////////////
	if (X < 5 && Y < 5) 
	{
		pWeightInputToConvolution1[Y*5+X] = (pWeightInputToConvolution1[Y*5+X])*OldWeightPercentage + 
													(pWeightInputToConvolution1[Y*5+X] - LearningRate * pPD_WeightInputToConvolution1[Y*5+X])*NewWeightPercentage;
	}
	__syncthreads();
...
*/	
		...
		{
			MaxCorrect = Correct;
			WriteWeightData(pWeightInputToConvolution1, pWeightSubsampling2ToOutput);
		}
